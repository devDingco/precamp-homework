...
function change_pw_2_color() {
  const pwinput1 = document.getElementById("비밀번호입력창").value;
  const pwinput2 = document.getElementById("비밀번호확인입력창").value;
  if (pwinput1 !== pwinput2) {
    document.getElementById("비밀번호확인입력창").style.border = "1px solid red";
  }
  else {
    document.getElementById("비밀번호확인입력창").style.border = "1px solid var(--gray-300, #c7c7c7)";
  }
}

function activate_signup() {
  const btn = document.getElementById("user-register-button")
  const img = document.querySelector("#register-img")
  img.src = "./assets/register-button-activated.png";
  btn.disabled = false;
}
...
function JS_회원가입() {
  ...
  const telcheck = /^010-\d{3,4}-\d{3,4}$/;
  if (!telcheck.test(내가입력한전화번호))
  {
    alert (`전화번호 형식이 올바르지 않습니다`);
    return;
  } 

  if (!mailcheck(내가입력한이메일))
  {
...
function JS_인증완료기능() {
  const 결과버튼 = document.getElementById("user-register-button");
  결과버튼.src = "./assets/verify-done.png";
}
